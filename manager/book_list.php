...
if (isset($_GET["query"]) && isset($_GET["searchby"])) {
  ...
  list($bookIds, $isbns, $bookNames, $authorNames, $publisherNames, $editions, $thumbnails, $prices) = bookSearch($query, $searchType) ;
  ...
  for($i = $startingIndex ; (($i < $pageSize*$page) && (($bookIds != null) && $i < count($bookIds))) ; $i++) {
    ...
    $bookHtml = $bookHtml.'<p class="book-inner-text" id="book-price">$'.$prices[$i].' per copy</p>' ;
...
//we are currently not in a search
  ...
  list($bookIds, $isbns, $bookNames, $authorNames, $publisherNames, $editions, $thumbnails, $prices) = getBooks() ;
...
<html>
	<head>
		<title>BookBoys - Manage Database</title>
    ...
    <div class="search" id="search-container">
      <p class="search-text" id="header">Manage Books:</p>
